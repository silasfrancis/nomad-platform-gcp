...
* **Octopus** - Octopus runs on SQL Server, so its restore procedure is different: stop the `octopus-server` Docker container (not a systemd service — Octopus runs via Docker Compose on `mgmt-vm`) to release its database connections, decompress the backup, run `sqlcmd`'s `RESTORE DATABASE ... WITH REPLACE` inside the SQL Server container, restart the `octopus-server` container, then query its health API to confirm it can access its data again. The SQL Server `sa` password is read from Secret Manager (`octopus-mssql-admin-password`) rather than Vault.
